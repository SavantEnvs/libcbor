/* Fleet policy (SPEC.md 6.1): disable LeakSanitizer preventively at BUILD time. ASan
   ...
   libcbor's harness drives cbor_load() over attacker-controlled CBOR. A rejected or
   truncated item leaves partially-built cbor_item_t trees owned by the caller, so LSan reports on
   inputs that are not the defect being hunted.
