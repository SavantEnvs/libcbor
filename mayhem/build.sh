...
cmake -S "$SRC" -B "$SAN_BUILD" \
    ...
    -D CMAKE_C_FLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS"
...
$CXX $SANITIZER_FLAGS $DEBUG_FLAGS -std=c++11 "-I$WORK/include" \
    "$SRC/mayhem/cbor_load_fuzzer.cc" /tmp/lsan_off.o $LIB_FUZZING_ENGINE "$LIBCBOR_A" \
...
$CXX $SANITIZER_FLAGS $DEBUG_FLAGS -std=c++11 "-I$WORK/include" \
    "$SRC/mayhem/cbor_load_fuzzer.cc" /tmp/standalone_main.o /tmp/lsan_off.o "$LIBCBOR_A" \
